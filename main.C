...
int main(int argc, char **argv){
  ...
  ///////////////
  // {{{
  Int_t uflag=0, wflag=0, Qflag=0, fflag=0, binFlag=0, Mflag=0, iflag=0, aflag=0;
  Int_t c;
  TString userNum = "", fname = "", outFile = "", binFile="";
  TString massRange="";
  Double_t M_min=0., M_max=16.;

  while ((c = getopt (argc, argv, "wb:u:f:Q:M:i:a:")) != -1) {
    switch (c) {
    case 'u':
      uflag = 1;
      userNum += optarg;
  ...
  if (!fflag) {
    ...
    Int_t nTree=0;
    TDirectory *dir = (TDirectory*)f1->Get(userEvent);
    TList *li = (TList*)dir->GetListOfKeys();
    TIter iter( li->MakeIterator() );
    while(TObject *obj = iter()){
      TKey *theKey = (TKey*)obj;
      if (strncmp (theKey->GetClassName(),"TTree",4) == 0){
	T1->Add( fname+"/"+userEvent+"/"+obj->GetName()+";"+
		 Form("%i", theKey->GetCycle()) );
	nTree++;
      }
    }
    f1->Close();
    cout << "Number of tree cycles: " << nTree << endl;
  }
  // }}}
